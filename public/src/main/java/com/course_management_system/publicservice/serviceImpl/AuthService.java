...
import com.course_management_system.publicservice.repository.AppUserRepository;
import com.course_management_system.publicservice.repository.StudentRepository;
import com.course_management_system.publicservice.security.JwtService;
import com.course_management_system.publicservice.security.TokenBlacklistService;
...
public class AuthService {
    ...
    public void signup(SignupRequest request) {
        if (appUserRepository.existsByEmail(request.getEmail()) || studentRepository.existsByEmail(request.getEmail())) {
    ...
    private AuthResponse buildAuthResponse(AppUser user) {
        Long studentId = studentRepository.findByEmail(user.getEmail()).map(Student::getId).orElse(null);
        return new AuthResponse(jwtService.generateToken(user.getEmail(), user.getRole()), "Bearer", user.getEmail(),
                user.getRole(), studentId);
    }
